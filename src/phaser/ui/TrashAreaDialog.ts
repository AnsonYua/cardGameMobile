...
export class TrashAreaDialog {
  ...
  private startPreview(card: any) {
    ...
    this.previewController.start((container) => {
      ...
      const textureKey = this.resolveFullTextureKey(preview.textureKey);
  ...
  private resolveFullTextureKey(textureKey?: string) {
    if (!textureKey) return textureKey;
    const key = String(textureKey);
    const fullKey = key.replace(/-thumb$/i, "");
    if (fullKey !== key && this.scene.textures.exists(fullKey)) return fullKey;
    return key;
  }
}
